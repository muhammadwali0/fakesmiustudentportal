...
adjustSelect = function (){
  document.getElementById('ps_select_parent').className="OldIEBox";
  document.getElementById('ps_select_box').className="selectOldIE";
  document.getElementById('ps_select_arrow').style.display="";
  var langsel = document.getElementById('ptlangsel');
  if (langsel){
	langsel.className="dropdownOldIE";
  	addEvent(langsel, "focus", function(){
	   document.getElementById('ps_select_parent').style.outline = "1px solid #ffff99";});
	addEvent(langsel, "blur", function(){
	   document.getElementById('ps_select_parent').style.outline = "none";
	});
  }
  var formfactorsel = document.getElementById('ptformfactor');
  if (formfactorsel){
	formfactorsel.className="dropdownOldIE";
  	addEvent(formfactorsel, "focus", function(){
	   document.getElementById('ps_select_parent').style.outline = "1px solid #ffff99";});
	addEvent(formfactorsel, "blur", function(){
	   document.getElementById('ps_select_parent').style.outline = "none";
	});
  }	
},
...
setErrorImg = function() {
  ...
  if (lErr || dErr || bErr) {
    try {
      var eAnc = document.getElementById("error_link");
      ...
      //eAnc.focus();  //if we focus here, JAWS will not notify user of the error text
    } catch (e) {}
  } else {
    setFocus();
  }
...
function updatePTCookie(jsonString, cookieName) {
    ...
    sCookieValue = sCookieValue.replace(/ /g, "");
    sCookieValue = sCookieValue.replace(/,/g, " ");
    setCookie(cookieName, sCookieValue, 1000, "/", sDomain, "");
}
...
function applyFormFactor() {   
    ...
    var url = window.location.href + "&ptformfactor=" + getFormFactorSize().formfactor;
...
function ptDeviceFeatures() {
    ...
    this.init = function () {
        ...
        var sValue = sValue = JSON.stringify(this.deviceFeatures);
